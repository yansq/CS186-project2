...
public class GHJOperator extends JoinOperator {
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        for (Record probeRecord : probeRecords) {
            ...
            if (hashMap.containsKey(value)) {
                List<Record> buildList = hashMap.get(value);
                for (Record buildRecord : buildList) {
                    if (probeFirst) {
                        this.joinedRecords.add(probeRecord.concat(buildRecord));
                    } else {
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        // (6 - 1): number of partitions
        // (6 - 2): max number of pages in one partition
        int sum = (6 -1) * (6 - 2) * 8 + 1;
        for (int i = 0; i < sum; i++) {
